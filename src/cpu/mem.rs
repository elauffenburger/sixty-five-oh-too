...
use std::io::Cursor;
use self::byteorder::{ LittleEndian, ByteOrder, ReadBytesExt };
use std::ops::Deref;
...
impl MemoryMap {
    ...
    pub fn crosses_page(addr_one: &u16, addr_two: &u16) -> bool {
...
mod test {
    ...
    #[test]
    pub fn test_crosses_page() {
        assert!(MemoryMap::crosses_page(&0x01FF, &0x0200));
        assert!(!MemoryMap::crosses_page(&0x01FF, &0x01FE));
    }
}